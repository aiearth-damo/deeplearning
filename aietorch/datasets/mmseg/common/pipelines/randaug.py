...
class Rand_Augment():
    ...
    def __init__(self, image_size = (1024, 1024), target_size = (1024,1024), Numbers=3, Magnitude=7, max_Magnitude=10, transforms=None, p=1.0):
        ...
        assert image_size[0] > 1 and image_size[1] > 1 and target_size[0] > 1 and target_size[1] > 1
    ...
    def randomCrop(self, image):
        if self.pad_c_half[0] or self.pad_c_half[1] or self.pad_r_half[0] or self.pad_r_half[1]:
            image = cv2.copyMakeBorder(image, top=self.pad_r_half[0], bottom=self.pad_r_half[1], left=self.pad_c_half[0],
                                     right=self.pad_c_half[1], borderType=cv2.BORDER_REFLECT_101)
        image = image[self.crop_beg[0]:self.crop_beg[0]+self.target_size[0], self.crop_beg[1]:self.crop_beg[1]+self.target_size[1]]
        return image
