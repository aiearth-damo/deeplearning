...
import numpy as np
import random
from aietorch.engine.mmseg.datasets.builder import PIPELINES
...
class ImageNetPolicy(object):
    ...
    def __init__(self, fillcolor=(128, 128, 128), fillcolor_mask=(255,), prob=1.0):
        ...
        self.policies = [
            ...
            SubPolicy(0.6, "solarize", 5, 0.6, "autocontrast", 5, fillcolor, fillcolor_mask),
            SubPolicy(0.8, "equalize", 8, 0.6, "equalize", 3, fillcolor, fillcolor_mask),
            SubPolicy(0.6, "posterize", 7, 0.6, "posterize", 6, fillcolor, fillcolor_mask),
            SubPolicy(0.4, "equalize", 7, 0.2, "solarize", 4, fillcolor, fillcolor_mask),

            SubPolicy(0.4, "equalize", 4, 0.8, "rotate", 8, fillcolor, fillcolor_mask),
            SubPolicy(0.6, "solarize", 3, 0.6, "equalize", 7, fillcolor, fillcolor_mask),
            SubPolicy(0.8, "posterize", 5, 1.0, "equalize", 2, fillcolor, fillcolor_mask),
            SubPolicy(0.2, "rotate", 3, 0.6, "solarize", 8, fillcolor, fillcolor_mask),
            SubPolicy(0.6, "equalize", 8, 0.4, "posterize", 6, fillcolor, fillcolor_mask),
            ...
            SubPolicy(0.0, "equalize", 7, 0.8, "equalize", 8, fillcolor, fillcolor_mask),
    ...
    def __call__(self, results):
        ...
        img, mask = self.policies[policy_idx](results['img'], results['gt_semantic_seg'])
        results['img'] = img
...
class SubPolicy(object):
    ...
    """
    def __init__(self, p1, operation1, magnitude_idx1, p2, operation2, magnitude_idx2, fillcolor=(128, 128, 128), fillcolor_mask=(255, )):
        ranges = {
    ...
    def _shearX(self, img, mask, magnitude):
        ...
        img = img.transform(img.size, Image.AFFINE, (1, magnitude * rand, 0, 0, 1, 0), Image.BILINEAR, fillcolor=self.fillcolor)
        mask = mask.transform(mask.size, Image.AFFINE, (1, magnitude * rand, 0, 0, 1, 0), Image.NEAREST, fillcolor=self.fillcolor_mask)

        return img, mask

    def _shearY(self, img, mask, magnitude):
        rand = random.choice([-1, 1])
        img = img.transform(img.size, Image.AFFINE, (1, 0, 0, magnitude * rand, 1, 0), Image.BILINEAR, fillcolor=self.fillcolor)
        mask = mask.transform(mask.size, Image.AFFINE, (1, 0, 0, magnitude * rand, 1, 0), Image.NEAREST, fillcolor=self.fillcolor_mask)
        return img, mask

    def _translateX(self, img, mask, magnitude):
        rand = random.choice([-1, 1])
        img = img.transform(img.size, Image.AFFINE, (1, 0, magnitude * img.size[0] * rand, 0, 1, 0), fillcolor=self.fillcolor)
        mask = mask.transform(mask.size, Image.AFFINE, (1, 0, magnitude * mask.size[0] * rand, 0, 1, 0), fillcolor=self.fillcolor_mask)
        return img, mask

    def _translateY(self, img, mask, magnitude):
        rand = random.choice([-1, 1])
        img = img.transform(img.size, Image.AFFINE, (1, 0, 0, 0, 1, magnitude * img.size[1] * rand), fillcolor=self.fillcolor)
        mask = mask.transform(mask.size, Image.AFFINE, (1, 0, 0, 0, 1, magnitude * mask.size[1] * rand), fillcolor=self.fillcolor_mask)
        return img, mask

    # from https://stackoverflow.com/questions/5252170/specify-image-filling-color-when-rotating-in-python-with-pil-and-setting-expand
    def _rotate(self, img, mask, magnitude):
        rot = img.convert("RGBA").rotate(magnitude)
        img = Image.composite(rot, Image.new("RGBA", rot.size, (self.fillcolor[0],) * 4), rot).convert(img.mode)
        rot_mask = mask.convert("RGBA").rotate(magnitude)
        mask = Image.composite(rot_mask, Image.new("RGBA", rot_mask.size, (self.fillcolor_mask[0],) * 4), rot_mask).convert(mask.mode)
        return img, mask
