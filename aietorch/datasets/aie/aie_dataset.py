...
from aietorch.datasets.datasets import NonGeoDataset, DatasetType
from aietorch.datasets.common.cv_ann_writer import UniversalCVAnnFileWriter
import aietorch.datasets.common.geo_transfer as  geo_transfer
import pandas as pd
import requests
import shutil
import tempfile
from aietorch.utils.colormap import create_pascal_label_colormap
...
class AIEDataSet(NonGeoDataset, BaseClient):
    ...
    def __init__(self, dataset_id, data_root=os.getcwd(), classes_filter=[]):
        ...
        lst_train_file = open(os.path.join(self.root_dir, self.__inner_lst_train_dir), 'r', encoding="UTF-8")
    ...
    def __preprocess(self, source_lst):
        ...
        for i in range(len(source_lst)):
            ...
            if (self.type == AIEDataSetType.__single__):
                ...
                ann_path = self.__inner_path_split(str(json_path).replace(".json", self.__seg_map_suffix))
    ...
    def __geojson_to_png(self, json_path, transform, png_path):
        ...
        ann_writer = UniversalCVAnnFileWriter(save_path=self.__get_real_path(png_path))
        geo_transfer.trans_annfile_to_image_coordinate(self.__get_real_path(json_path),
                                                       self.__get_real_path(transform),
...
class BinaryChangeDetDataset(AIEDataSet):
    def __init__(self, dataset_id, data_root=os.getcwd(), classes_filter=[]):
        ...
        from aietorch.datasets.mmseg import ChangeDetDataset
...
class LandcoverDataset(AIEDataSet):
    def __init__(self, dataset_id, data_root=os.getcwd(), classes_filter=[]):
        super().__init__(dataset_id, data_root, classes_filter)
        from aietorch.datasets.mmseg.landcover.landcover import LandcoverLoader
...
class TargetExtractionDateset(AIEDataSet):
    def __init__(self, dataset_id, data_root=os.getcwd(), classes_filter=[]):
        super().__init__(dataset_id, data_root, classes_filter)
        from aietorch.datasets.mmseg.common.remote_sensing import RemoteSensingBinary
