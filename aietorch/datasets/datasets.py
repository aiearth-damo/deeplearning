...
from aietorch.utils.file import list_dir
...
class NonGeoDataset(Dataset):
    ...
    def generate_mapping_list(self):
        ...
        if self._dataset_type == DatasetType.double_img:
            ...
            img2_list = list_dir(self._img2_dir, self._img_suffix, abspath=True)
        
        for ann_path in ann_list:
            file_name_without_ext = os.path.splitext(os.path.basename(ann_path))[0]
            img_path = os.path.join(os.path.abspath(self._img_dir), file_name_without_ext+self._img_suffix)
            if self._dataset_type == DatasetType.single_img:
                ...
                img2_path = os.path.join(os.path.abspath(self._img2_dir), file_name_without_ext+self._img_suffix)
...
class ChangeDetNonGeoCustomDataset(NonGeoCustomDataset):
    def __init__(self, classes: tuple or list, 
        ...
        super().__init__(classes, img_shape, ann_shape, dataset_type, img_dir, img2_dir, img_suffix, ann_dir, ann_suffix, img_ann_mapping_list, img_dtype, ann_dtype)
...
class TargetExtractionNonGeoCustomDataset(NonGeoCustomDataset):
    def __init__(self, classes: tuple or list, 
        ...
        img2_dir = None
        super().__init__(classes, img_shape, ann_shape, dataset_type, img_dir, img2_dir, img_suffix, ann_dir, ann_suffix, img_ann_mapping_list, img_dtype, ann_dtype)
        from .mmseg.common.remote_sensing import RemoteSensingBinary
