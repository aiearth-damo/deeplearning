...
from aietorch.engine.mmseg.apis import show_result_pyplot
from aietorch.engine.mmseg.apis.inference import LoadImage
from aietorch.engine.mmseg.datasets.pipelines import Compose
from aietorch.engine.mmseg.models import build_segmentor
...
def pytorch2onnx(
    ...
    if verify:
        ...
        if dynamic_export:
            ...
            img_list = [
                nn.functional.interpolate(_, scale_factor=1.5) for _ in img_list
            ]
        ...
        print("pytorch_result.max()", pytorch_result.max(), pytorch_result.shape)
        print("onnx_result.max()", onnx_result.max(), onnx_result.shape)
        pytorch_result = pytorch_result[0]
        pytorch_result = pytorch_result * 20
        cv2.imwrite("./pytorch_result.png", pytorch_result)
        onnx_result = onnx_result.argmax(axis=0)
        onnx_result = onnx_result * 20
        cv2.imwrite("onnx_result.png", onnx_result)
        # compare results
...
def to_onnx(cfg, output_path, checkpoint_path=None, shape=(1024, 1024)):
    ...
    if checkpoint_path:
        checkpoint = load_checkpoint(
            ...
            revise_keys=[
                (r"^module\.", ""),
                (r"^architecture\.model\.", ""),
                (r"^architecture1\.model\.", ""),
            ],
        )
