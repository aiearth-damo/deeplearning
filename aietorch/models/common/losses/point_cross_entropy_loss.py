...
from aietorch.engine.mmseg.models.builder import LOSSES
from aietorch.engine.mmseg.models.losses.utils import get_class_weight, weight_reduce_loss
...
def fuse_cross_entropy(
    ...
    if mask_images.sum() == 0:
        ...
        loss_image = loss[mask_images].mean() * (B - points_batch.sum()) / B
    if mask_points.sum() == 0:
        loss_point = 0.0
    else:
        loss_point = loss[mask_points].mean() * points_batch.sum() / B
    loss = fuse_lambda * loss_image + (1 - fuse_lambda) * loss_point

    return loss


def only_point_cross_entropy(
    pred,
    label,
    weight=None,
    class_weight=None,
...
class PointCrossEntropyLoss(nn.Module):
    ...
    def __init__(
        ...
        if self.use_sigmoid:
            ...
            self.cls_criterion = fuse_cross_entropy
        else:
            self.cls_criterion = only_point_cross_entropy
    ...
    def forward(
        ...
        **kwargs
        ...
        if self.class_weight is not None:
            ...
            class_weight = None
        if self.fuse:
            kwargs["point_num"] = self.point_num
            kwargs["fuse_lambda"] = self.fuse_lambda
