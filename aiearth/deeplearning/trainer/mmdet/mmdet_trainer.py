...
from .mmdet_utils import mmdet_base_config
...
class AIEMMdetCloudModel(AIEModel):
    CLOUD_MODEL_TYPE = None

    def __init__(self, mmdet_config, onnx_shape=None):
        self.mmdet_config = mmdet_config
        self.onnx_shape = None

    def get_mean(self):
        if type(self.mmdet_config.data.train) in (list, tuple):
            train_pipeline = self.mmdet_config.data.train[0]["pipeline"]
        else:
            train_pipeline = self.mmdet_config.data.train["pipeline"]
    ...
    def get_onnx_shape(self):
        if self.onnx_shape is not None:
    ...
    def get_classes(self):
        return self.mmdet_config.aie_classes
    ...
    def is_fp16(self):
        if "Fp16OptimizerHook" in self.mmdet_config.optimizer_config:
            return True
        return False
...
class MMDetTrainer(Trainer):
    ...
    def set_base_cfg_from_file(self, config_file):
        self.base_cfg = mmdet_base_config()
    ...
    def __setup_nongeodataset(self, dataset: NonGeoDataset, data_tmpl, data_type=None):
        ...
        data_root, split = self.__nongeodataset_to_mmdet_splitfile(dataset, data_type)
        data_cfg.merge_from_dict(
            dict(data_root=data_root, split=split, img_dir="/", ann_dir="/")
        )
        return data_cfg

    def __nongeodataset_to_mmdet_splitfile(
    ...
    def build_model(self):
        ...
        rfnext_init_model(model, cfg=self.cfg)
