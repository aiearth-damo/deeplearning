...
class PolygonMasks(BaseInstanceMasks):
    ...
    def __init__(self, masks, height, width, poly_weights=None):
    ...
    def __getitem__(self, index):
        ...
        return PolygonMasks(masks, self.height, self.width, self.poly_weights)
    ...
    def rescale(self, scale, interpolation=None):
        ...
        new_w, new_h = mmcv.rescale_size((self.width, self.height), scale)
        if len(self.masks) == 0:
            rescaled_masks = PolygonMasks([], new_h, new_w, self.poly_weights)
        else:
    ...
    def resize(self, out_shape, interpolation=None):
        """see :func:`BaseInstanceMasks.resize`"""
        if len(self.masks) == 0:
            resized_masks = PolygonMasks([], *out_shape, poly_weights=self.poly_weights)
        else:
            ...
            resized_masks = PolygonMasks(
                resized_masks, *out_shape, poly_weights=self.poly_weights
            )
    ...
    def flip(self, flip_direction="horizontal"):
        ...
        assert flip_direction in ("horizontal", "vertical", "diagonal")
        if len(self.masks) == 0:
            flipped_masks = PolygonMasks([], self.height, self.width, self.poly_weights)
        else:
            flipped_masks = []
            for poly_per_obj in self.masks:
            ...
            flipped_masks = PolygonMasks(
                flipped_masks, self.height, self.width, self.poly_weights
            )
    ...
    def crop(self, bbox):
        ...
        if len(self.masks) == 0:
            cropped_masks = PolygonMasks([], h, w, self.poly_weights)
        else:
            cropped_masks = []
            for poly_per_obj in self.masks:
            ...
            cropped_masks = PolygonMasks(cropped_masks, h, w, self.poly_weights)
        return cropped_masks

    def pad(self, out_shape, pad_val=0):
        """padding has no effect on polygons`"""
        return PolygonMasks(self.masks, *out_shape, poly_weights=self.poly_weights)
    ...
    def crop_and_resize(
        ...
        if len(self.masks) == 0:
            return PolygonMasks([], out_h, out_w, self.poly_weights)
        ...
        return PolygonMasks(resized_masks, *out_shape, poly_weights=self.poly_weights)
    ...
    def translate(
        ...
        if len(self.masks) == 0:
            translated_masks = PolygonMasks(
                [], *out_shape, poly_weights=self.poly_weights
            )
        else:
            translated_masks = []
            for poly_per_obj in self.masks:
            ...
            translated_masks = PolygonMasks(
                translated_masks, *out_shape, poly_weights=self.poly_weights
            )
    ...
    def shear(
        ...
        if len(self.masks) == 0:
            sheared_masks = PolygonMasks([], *out_shape, poly_weights=self.poly_weights)
        else:
            sheared_masks = []
            if direction == "horizontal":
            ...
            sheared_masks = PolygonMasks(
                sheared_masks, *out_shape, poly_weights=self.poly_weights
            )
    ...
    def rotate(self, out_shape, angle, center=None, scale=1.0, fill_val=0):
        ...
        if len(self.masks) == 0:
            rotated_masks = PolygonMasks([], *out_shape, poly_weights=self.poly_weights)
        else:
            rotated_masks = []
            ...
            for poly_per_obj in self.masks:
                ...
                for p in poly_per_obj:
                    ...
                    coords = np.stack([p[:, 0], p[:, 1]], axis=1)  # [n, 2]
            ...
            rotated_masks = PolygonMasks(
                rotated_masks, *out_shape, poly_weights=self.poly_weights
            )
    ...
    def areas(self):
        ...
        for polygons_per_obj in self.masks:
            ...
            for p in polygons_per_obj:
                ...
                area_per_obj += self._polygon_area(p[:, 0], p[:, 1])
