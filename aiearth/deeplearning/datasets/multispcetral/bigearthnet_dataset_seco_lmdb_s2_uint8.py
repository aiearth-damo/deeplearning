...
class LMDBDataset(Dataset):
    ...
    def norm(self,sample):
        # print(sample.shape)
        #128*128*12
        maxv=np.max(sample,axis=2)
        minv=np.min(sample,axis=2)
        maxv=maxv.reshape(128,128,1)
        minv=minv.reshape(128,128,1)
        sample=(sample-maxv)/(maxv-minv)
        # print(sample.shape)
        # exit()
        return sample
    ...
    def __getitem__(self, index):
        ...
        # sample=np.array(cv2.convertScaleAbs(sample))
